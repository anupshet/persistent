...
// const allTest = '//br-analyte-summary-view/section/div[2]/div[1]/table/tbody/tr[1]/td[1]';
...
const calenderSelect = 'mat-calendar-arrow';
...
// const displatDate = './/h6[@class="mat-h6"][1]';
...
const datepick = 'mat-icon-button';
const previousDate = 'mat-calendar-previous-button mat-icon-button';
...
export class MultiSummary1 {
  ...
  verifySubmitButtonEnabled() {
    let status = false;
    return new Promise((resolve) => {
      const submitButton1 = element(by.id(submitButton));
      submitButton1.isDisplayed().then(function () {
  ...
  enterMeanSDPointValues(dataMap) {
    let status = false;
    return new Promise((resolve) => {
      dashBoard.waitForElement();
      dataMap.forEach(function (key, value) {
  ...
  clickSubmitButton() {
    let status = false;
    return new Promise((resolve) => {
      const submit = findElement(locatorType.ID, submitButton);
    ...
    return new Promise((resolve) => {
      ...
      try {
        if (analyteName === 'Amylase') {
          noDataEle = findElement(locatorType.XPATH, '//p[contains(text(),"' + analyteName + '")]'
