...
/*
fs.readFile('./e2e/qcp-central/test-data/pbi-213862.json', (err, data) => {
  if (err) { throw err; }
...
// For execution on QA
fs.readFile('./e2e/qcp-central/test-data/pbi-213862_QA.json', (err, data) => {
  if (err) { throw err; }
  const pointData = JSON.parse(data);
  jsonData = pointData;
});
...
describe('PBI 213862: Migration Tool Regression: 1-1 Mapping (With Transformer & Without Panel)', function () {
  ...
  it('PBI 213862: To check Unity-Next Account Creation status for a migrating lab', function () {
    library.logStep('PBI 213862: To check Unity-Next Account Creation status for a migrating lab');
  ...
  it('PBI 213862: To create an account in Unity Next for the migrating lab', function () {
    library.logStep('PBI 213862: To create an account in Unity Next for the migrating lab');
  ...
  it('PBI 213862: To retrieve the Configurations', function () {
    library.logStep('PBI 213862: To retrieve the Configurations');
    loginEvent.loginToQCP(jsonData.URL, jsonData.Username, jsonData.Password).then(function (loggedIn) {
      expect(loggedIn).toBe(true);
    });
    qcp.expandMainMenu(jsonData.Menu).then(function (navigatedToMenu) {
  ...
  it('PBI 213862: To Migrate the Transformers and to initiate the QC Data migration', function () {
    library.logStep('PBI 213862: To Migrate the Transformers and to initiate the QC Data migration');
    loginEvent.loginToQCP(jsonData.URL, jsonData.Username, jsonData.Password).then(function (loggedIn) {
      expect(loggedIn).toBe(true);
    });
    qcp.expandMainMenu(jsonData.Menu).then(function (navigatedToMenu) {
    ...
    migrationConfig.clickbtnContinueToResultsRetrievalButton().then(function (continueMigrationCliecked) {
  ...
  // Please activate the migrated lab user account with password "Welcome1" before running this test
  it('PBI 213862: To validate the migrated data', function () {
    library.logStep('PBI 213862: To validate the migrated data');
    ...
    connect.verifyInstructionsNameDisplayed(jsonData.MigratedTransformer).then(function (verified) {
      expect(verified).toBe(true);
    });
