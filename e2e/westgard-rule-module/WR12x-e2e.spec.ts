...
library.parseJson('./JSON_data/WR12x.json').then(function(data) {
  jsonData = data;
});
...
describe('Westgard Rules:- 12x', function () {
  ...
  it('Test Case 5: To check if the Warning is shown for the multilevel data which violates 12x rule for values below Mean (Across runs) @P1',
    function () {
      library.logStep('Test Case 5: To check if the Warning is shown for the multilevel data which violates the 12x rule for values below Mean (Across runs)');
  ...
  it('Test Case 9:- To check if the Rejection is shown for the data which violates the 12x rule for values above Mean (within run) @P1',
    function () {
  ...
  it('Test Case 17: To verify that any value above Mean can be added for multilevel data after disabling the 12x Rule (Across runs) @P1',
    function () {
      ...
      analyteSettings.selectKxRule('12x', 'disable').then(function (rule1Selected) {
        expect(rule1Selected).toBe(true);
      });
      analyteSettings.clickUpdateButton().then(function (clicked) {
        expect(clicked).toBe(true);
