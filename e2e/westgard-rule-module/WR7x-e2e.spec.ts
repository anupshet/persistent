...
library.parseJson('./JSON_data/WR7x.json').then(function(data) {
  jsonData = data;
});
...
describe('Westgard Rules:- 7x', function () {
  ...
  it('Test Case 5:- To check if Warning is shown for multilevel data which violates 7x rule for values below Mean (Across runs) @P1',
    function () {
  ...
  it('Test Case 9:- To check if the Rejection is shown for the data which violates the 7x rule for values above Mean (within run) @P1',
    function () {
      library.logStep('Test Case 9:- To check if the Rejection is shown for the data which violates the 7x rule for values above Mean (within run)');
      newLabSetup.navigateTO(jsonData.DepartmentName).then(function (dept) {
        expect(dept).toBe(true);
      });
      newLabSetup.navigateTO(jsonData.InstrumentName).then(function (inst) {
      ...
      analyteSettings.selectKxRule('7x', 'reject').then(function (rule1Selected) {
        expect(rule1Selected).toBe(true);
      });
      analyteSettings.clickUpdateButton().then(function (clicked) {
        expect(clicked).toBe(true);
  ...
  it('Test Case 17: To verify that any value above Mean can be added for multilevel data after disabling the 7x Rule (Across runs) @P1',
    function () {
  // tslint:disable-next-line: max-line-length
      library.logStep('Test Case 17: To verify that any value above Mean can be added for multilevel data after disabling the 7x Rule (Across runs)');
      newLabSetup.navigateTO(jsonData.DepartmentName).then(function (dept) {
        expect(dept).toBe(true);
      });
      newLabSetup.navigateTO(jsonData.InstrumentName).then(function (inst) {
      ...
      analyteSettings.selectKxRule('7x', 'disable').then(function (rule1Selected) {
        expect(rule1Selected).toBe(true);
      });
      analyteSettings.clickUpdateButton().then(function (clicked) {
        expect(clicked).toBe(true);
