...
library.parseJson('./JSON_data/WR8x.json').then(function(data) {
  jsonData = data;
});

describe('Westgard Rules:- 8x', function () {
  ...
  let multiplyL1, multiplyL2, firstAddL1, firstAddL2, secondAddL1, secondAddL2,
    thirdAddL1, thirdAddL2, fourthAddL1, fourthAddL2, l1Value, l2Value;
  ...
  it('Test Case 2: To check if the Warning is shown for the data which violates the 8x rule for values above Mean (within run) @P1',
    function () {
      library.logStep('Test Case 2: To check if the Warning is shown for the data which violates the 8x rule for values above Mean (within run)');
      ...
      analyteSettings.selectKxRule('8x', 'warning').then(function (rule1Selected) {
  ...
  it('Test Case 12: To check if the Rejection is shown for the multilevel data which violates the 8x rule for values below Mean (Across runs) @P1',
    function () {
      library.logStep('Test Case 12: To check if the Rejection is shown for the multilevel data which violates the 8x rule for values below Mean (Across runs)');
      newLabSetup.navigateTO(jsonData.DepartmentName).then(function (dept) {
        expect(dept).toBe(true);
      });
      newLabSetup.navigateTO(jsonData.InstrumentName).then(function (inst) {
  ...
  it('Test Case 17: To verify that any value above Mean can be added for multilevel data after disabling the 8x Rule (Across runs) @P1',
    function () {
      library.logStep('Test Case 17: To verify that any value above Mean can be added for multilevel data after disabling the 8x Rule (Across runs)');
      newLabSetup.navigateTO(jsonData.DepartmentName).then(function (dept) {
        expect(dept).toBe(true);
      });
      newLabSetup.navigateTO(jsonData.InstrumentName).then(function (inst) {
      ...
      analyteSettings.selectKxRule('8x', 'disable').then(function (rule1Selected) {
        expect(rule1Selected).toBe(true);
      });
      analyteSettings.clickUpdateButton().then(function (clicked) {
        expect(clicked).toBe(true);
