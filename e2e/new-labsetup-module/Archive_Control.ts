...
const fs = require('fs');
let jsonData;
fs.readFile('./JSON_data/Archive_Control.json', (err, data) => {
  if (err) { throw err; }
...
describe('Test Suite: To verify archive functionality for controls', function () {
  ...
  it('To archive control and verify the archive functionlaity', function () {
    ...
    /**
     * Need to discuss this method with Amruta
     */
    /* archivingLots.verifyLeftNavigationToggleFunctionality('Ferritin').then(function (verified){
      expect(verified).toBe(true);
    });
    */
  });
