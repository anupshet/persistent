...
library.parseJson('./JSON_data/advancelj-210174.json').then(function(data) {
  jsonData = data;
});

describe('Test Suite 210174:Render triangles for points out of range of +3SD/-3SD.', function () {
  ...
  it('Test Case 1:To Verify rendering of upward traingles, when point goes above the range of +3sd @P1', function () {
    imageName = '+3sdPoints';
    details = 'Rendered a upward traingles, when point goes above the range of +3sd';
    ...
    advancedLj.clickOnLeftArrow().then(function(clicked){
      expect(clicked).toBe(true);
    });
  ...
  it('Test Case 2:To Verify rendering of downward traingles, when point goes below the range of -3sd ', function () {
    details = 'Rendered a downward traingles, when point goes below the range of -3sd';
    imageName = '-3sd Points';
    labsetup.navigateTO(jsonData.Department).then(function (navigated) {
      expect(navigated).toBe(true);
    });
    labsetup.navigateTO(jsonData.Instrument).then(function (navigated) {
