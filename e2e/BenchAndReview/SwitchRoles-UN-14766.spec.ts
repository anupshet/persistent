...
describe("Test Suite: UN-17002 : Bench Review: Analyte details/options for the run", function () {
  ...
  it("TC-1 : Verify access to Supervisor review via dashboard card", function () {
  ...
  it("TC-2 : Verify that AUM and LUM roles cannot access Supervisor review", function () {
    out.signOut().then(function (status) {
      expect(status).toBe(true);
    });
    loginEvent.loginToApplication(jsonData.URL, jsonData.AUMUserName,
      jsonData.AUMPassword, jsonData.FirstName).then(function (loggedIn) {
        expect(loggedIn).toBe(true);
      });
    benchAndReview.navigateToQCResults().then(function (navigated) {
      expect(navigated).toBe(true);
    ...
    loginEvent.loginToApplication(jsonData.URL, jsonData.LUMUserName,
      jsonData.LUMPassword, jsonData.FirstName).then(function (loggedIn) {
        expect(loggedIn).toBe(true);
      });
    benchAndReview.navigateToQCResults().then(function (navigated) {
      expect(navigated).toBe(true);
