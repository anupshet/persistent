...
@Injectable()
export class ConnectivityMappingApiService extends ApiService {
  ...
  constructor(http: HttpClient, config: ConfigService, store: ngrxStore.Store<fromRoot.State>,
    ...
    this.apiUrl  = (<ApiConfig>config.getConfig('api')) ? (<ApiConfig>config.getConfig('api')).connectivityUrl : null;
 }
}
