...
import { NotificationApiService } from 'src/app/shared/navigation/services/notificationApi.service';
...
describe('PreviewReportComponent', () => {
  ...
  const mockReportingService = jasmine.createSpyObj([
    'downloadPdfData',
    'create',
    'createTimeoutReport'
  ]);
  const mockReportHelperService = jasmine.createSpyObj([
    'getReportRequestInfo',
    'getReportLevelFromEntityType'
  ]);
  ...
  beforeEach(() => {
    ...
    component.data = {
      reportUrl: {
        reportResultInfo: {
          pdfFileUrl: 'pdf-fake-url'
        }
      },
      node: mockStore.navigation.selectedNode,
      reportType: '',
      yearMonth: '',
      lab: { comments: '', signedBy: '', commentsDict: '', signedOn: '' },
      langCode: '',
      ancestors: [mockStore.navigation.selectedLeaf, mockStore.navigation.selectedNode],
      nodeWithGrandchildren: ''
    };
  ...
  it('should close modal and dispatch remove notification when click on save button',
    inject([MatDialogRef, Store, ReportingService, ReportHelperService], (matDialogRef,
      store: Store, reportingService: ReportingService) => {
      component.data.fromNotification = true;
      reportingService.createTimeoutReport = jasmine.createSpy().and.returnValue(of(true));
      component.data.notificationId = 'TestNotificationId';
      const dispatchSpy = spyOn(store, 'dispatch');
      const spy = spyOn(matDialogRef, 'close');
      component.onSave();
      expect(dispatchSpy).toHaveBeenCalledWith(notificationActions.dismissNotification({ notificationUuid: 'TestNotificationId' }));
      expect(spy).toHaveBeenCalled();
    })
  );

  it('should call logErrorToBackend when remove notification dispatch fails on click of save button',
    inject([Store, ReportingService, ReportHelperService, ErrorLoggerService], (store: Store,
      reportingService: ReportingService) => {
      component.data.fromNotification = true;
      reportingService.createTimeoutReport = jasmine.createSpy().and.returnValue(of(true));
      component.data.notificationId = 'TestNotificationId';
      reportingService.createTimeoutReport = jasmine.createSpy().and.returnValue(of(true));
      const spyOnErrorLog = spyOn(mockErrorLoggerService, 'logErrorToBackend');
      spyOn(store, 'dispatch').and.throwError('');
      component.onSave();
      expect(spyOnErrorLog).toHaveBeenCalled();
    })
  );
});
